/**
 * This plugin reads and manipulate the data of tiles and invokes tile plugin to add each tile on the canvas
 * @class tiles
 * @extends Plugin
...
Plugin.extend({
    ...
    initPlugin: function(data) {
        ...
        if (this._parent._controller && model) {
            ...
            if (levelIndex) {
                ...
                if ((currentLevel == "level4" && (previousLevel == "level1" || previousLevel == "level2" || previousLevel == "level3" || previousLevel == "level5" || previousLevel == "level6")) || (previousLevel == "level4" && (currentLevel == "level1" || currentLevel == "level2" || currentLevel == "level3" || currentLevel == "level5" || currentLevel == "level6"))) {
                    isArrayChange = true;
                } else {
                    isArrayChange = false;
                }
    ...
    /**
     * This method creates grids and invoke tile plugin for each cell in the grid
     * @param {array} value, it is the array of objects (Object containing info about each cell)
     * @memberof tiles
     */
    ...
    /**
     * This method creates each cell object with the dimenstion for a grid 
     * @param {number} cols,  no of columns
     * @param {number} rows, no of rows
     * @param {object} containersize, its an object which has 2 properties width as w and height as h (value for these parameter is number without any unit like px or %)
     * @return {array} gridObjects, array of objects (each object has the info for a cell)
     * @memberof tiles
     */
