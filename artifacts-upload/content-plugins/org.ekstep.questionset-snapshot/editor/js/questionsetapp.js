    ...
    .controller('questionsetcontroller', ['$scope', '$injector', 'instance', function($scope, $injector, instance) {
        ...
        ctrl.activity = {
         ...
         ctrl.loadSelectedQuestions = function() {
            instance.data.questionnaire && ctrl.setupQuestionsConfig();
        }
        ...
        ecEditor.getService('language').getLanguages(function(err, respLan) {
         ...
         ctrl.setupQuestionsConfig = function() {
            ctrl.activityOptions = {title: ctrl.activityOptions.title, shuffle: ctrl.activityOptions.shuffle, showImmediateFeedback: ctrl.activityOptions.showImmediateFeedback, concepts: ctrl.activityOptions.concepts }; 
            ctrl.cart = {"items": instance.data.questionnaire.items[instance.data.questionnaire.item_sets[0].id] };
            ctrl.activityOptions.total_items = ctrl.cart.items.length;
            ctrl.activityOptions.max_score = ctrl.activityOptions.total_items;
            ctrl.activityOptions.range = ecEditor._.times(ctrl.activityOptions.total_items).splice(1);
        ...
        ctrl.addItemActivity = function() {
            ...
            var _assessmentData = {};
            _assessmentData["data"] = JSON.stringify(instance.manifest.editor.questionConfig.dataplugin)
            _assessmentData["config"] =JSON.stringify(instance.manifest.editor.questionConfig.configdata);
    
            ecEditor.dispatchEvent(instance.manifest.id+ ':create', _assessmentData);
            //ecEditor.dispatchEvent(instance.manifest.id + ':renderQuiz', _assessmentData);
            //  console.log("ctrl.activityOptions"+ctrl.activityOptions);
