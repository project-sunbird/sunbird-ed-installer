 ...
 */
ftEditorApp.controller('romanmtfController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
    $scope.items = [];
    var config = {
    ...
    $scope.addQuestion = function () {
        ...
        newItem.model.mh = $scope.config.selectedConfig.itemData.mh;
        config.locales = {
        ...
        $http.get(commoni18nFile).then(function (response) {
            ...
            $http.get(localeFile).then(function (response) {
                ...
                newItem.model.hintMsg = localeData[contentLocaleVal][$scope.config.selectedConfig.itemData.HintID];
    ...
    var init = function () {
        ...
        $scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
            instance.manifest.ver, '/editor/roman.mtf.json');
