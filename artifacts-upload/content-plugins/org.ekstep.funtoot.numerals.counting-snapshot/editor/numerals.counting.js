 ...
 */
ftEditorApp.controller('numberCountingController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
	...
	var item = {
		...
		model: {
			...
			variables: {
				...
				$totalNums: "0",
				$skip: '',
				$nums: "getNumberSequence($expr, $totalNums, $skip)"
			}
		},
		concepts: {
			identifier: 'C7',
	...
	$scope.addQuestion = function () {
		...
		// The matrix is of the form [3x3]. This is parsed to get rows and columns
		var matrix = $scope.config.selectedConfig.itemData.Matrix.split('x');
		newItem.model.rows = parseInt(matrix[0].trim());
		newItem.model.cols = parseInt(matrix[1].trim());
		...
		newItem.model.variables.$totalNums = matrix[0].trim() + '*' + matrix[1].trim();
		// populate the editable boxes through the input mask
		newItem.model.mask = eval($scope.config.selectedConfig.itemData.Mask);
		newItem.model.skip = eval($scope.config.selectedConfig.itemData.Skip);
		newItem.model.variables.$skip = newItem.model.skip;
	...
	var init = function () {
		...
		$scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
			instance.manifest.ver, '/editor/numerals.counting.json');
