...
org.ekstep.funtoot.common.extend({
    ...
    initPlugin: function (data) {
        ...
        if (!model.isProcessed) {
            ...
            model.model.hintMsg = new org.ekstep.generators().replaceVariables(model.i18n[model.model.langId][model.model.hintMsg], variables);
        ...
        questionText = new org.ekstep.generators().replaceVariables(questionText, variables);
        ...
        _.each(model.model.premises, function (premise, i) {
            ...
            lhsContainer.onMicroHint = function (e) {
                ...
                mhData.w = 80;
                mhData.h = 60;
                mhData.content = new org.ekstep.generators().replaceVariables(mhModel.mh, this._stage.getController("item").getModelValue("variables"));
            ...
            if (model.lhs_options[i].value.type == "text") {
                var textObj = {
                    ...
                    x: 0,
                    y: 0,
                    content: new org.ekstep.generators().replaceVariables(model.i18n[model.model.langId][premise.text], variables),
                    valign: "middle"
                }
            ...
            if (!data.isSolution) {
                ...
                var optObj = PluginManager.getPluginObject("lhsShape[" + i + "]");
                optObj._self.graphics.setStrokeDash([10, 3]);
                optObj._self.graphics.setStrokeStyle(2);
        ...
        _.each(model.model.premises, function (premise, i) {
            ...
            PluginManager.invoke('image', imgObj, textContainer, instance._stage, instance._theme);
        });
    },
    /**
    ...
    buildOption: function (opParent, config) {
        ...
        if (config.stroke) mtfOptData["stroke"] = config.stroke;
    ...
    onSubmit: function (evt, instance) {
        ...
        if (this.onEvaluate(instance)) {
            ...
            for (var i = 0; i < rhs_options.length; i++) {
                ...
                if (!_.isUndefined(rhs_options[i].selected)) {
                    ...
                    if (rhs_options[i].selected == rhs_options[i].answer) {
                        ...
                        if (Array.isArray(model.model.premises[lhs_selected].mmc))
                            result.mmc.push.apply(result.mmc, model.model.premises[lhs_selected].mmc);
                        else
                            result.mmc.push(model.model.premises[lhs_selected].mmc);
                    }
                    var res = {};
                    res[rhs_options[i].value.resindex] = rhs_options[i].selected;
            ...
            for (var j = 0; j < rhs_options.length; j++) {
                ...
                if (!lhs_options[j].isCorrect && areMicrohintsEnabled) {
