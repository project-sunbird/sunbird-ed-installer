//@ sourceURL=multiplicationvertical-editor.js
'use strict';
/**
 ...
 */
ftEditorApp.controller('multiplicationverticalController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
	$scope.items = [];
	var config = {
		title: 'Multiply',
		count: 0
	...
	var item = {
		...
		type: 'mtf',
		template_id: '',
		template: 'multiplicationvertical',
	...
	var init = function () {
		...
		$scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
			instance.manifest.ver, '/editor/multiply.vertical.json');
