//@ sourceURL=additionhorizontal-editor.js
'use strict';
/**
 ...
 */
ftEditorApp.controller('AdditionHorizontalController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
	$scope.items = [];
	var config = {
		title: 'Horizontal Addition',
		count: 0
	};
	var item = {
		langid: 'en',
		identifier: 'additionhorizontal',
		language: ["English"],
		qid: '',
		subject: "NUM",
		grade: 0,
	...
	$scope.addQuestion = function () {
		...
		newItem.model.numberType = $scope.config.selectedConfig.itemData.Type == "Decimal Addition" ? "Decimal" : "Integer";
		...
		$http.get(commoni18nFile).then(function (response) {
			...
			$http.get(localeFile).then(function (response) {
				...
				var qTitle = newItem.model.numberType == "Decimal" ? "DEC_QUESTION_TITLE" : "QUESTION_TITLE"
				newItem.title = config.title = localeData[contentLocaleVal][qTitle];
	...
	var init = function () {
		...
		$scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
			instance.manifest.ver, '/editor/additionhorizontal.json');
