//@ sourceURL=readsettimebase-renderer.js
...
org.ekstep.funtoot.common.extend({
    _type: 'org.ekstep.plugins.funtoot.readsettimebase',
    initPlugin: function (data) {
        ...
        var clkContainer = PluginManager.getPluginObject(this._ftContentContainerId);
        ...
        model.time = time;
        if (model.model.question.includes("SET")) {
        ...
        for (var s = 0; s < time.length; s++) {
            ...
            if (!data.isSolution) {
                var fibData = {
                    e: i18n.translateNumber(time[s], nlangId).displayValue,
                    u: z == 0 ? time[s] : '',
                    w: model.model.question.includes("SET") ? false : true,
        ...
        function drawClock(instance) {
            ...
            var plugData = {
                ...
                nlangid: "en",
                clicked: click,
                hours: model.model.question.includes("READ") || data.isSolution ? model.time[0] : model.set[0],
                min: model.model.question.includes("READ") || data.isSolution ? model.time[1] : model.set[1],
                seconds: model.model.question.includes("READ") || data.isSolution ? model.time[2] : model.set[2],
                isSolution: data.isSolution
            };
    ...
    onSubmit: function (evt, instance) {
        ...
        if (this.onEvaluate(instance)) {
            ...
            if (model.model.question.includes("READ")) {
                _.each(model.model.fibModels, function (fib) {
                    var re = {};
                    re[fib.key] = fib.u
                    result.resValues.push(re);
                    if (res) {
                        var user = i18n.toNumber(fib.u);
                        var expected = i18n.toNumber(fib.e);
                        if (parseInt(user) != parseInt(expected)) {
                            res = false;
                        } else {
                            res = true;
                        }
