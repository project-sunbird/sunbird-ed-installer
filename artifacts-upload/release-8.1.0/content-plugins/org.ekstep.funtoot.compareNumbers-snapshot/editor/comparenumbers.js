//@ sourceURL=compareNumbers-editor.js
'use strict';
/**
 ...
 */
ftEditorApp.controller('compareNumbersController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
	$scope.items = [];
	var config = {
		title: 'Compare the numbers using correct symbol',
		count: 0
	};
	var item = {
		langid: 'en',
		language: ["English"],
		identifier: 'compare_numbers',
		qid: '',
		subject: "NUM",
		grade: 0,
		gradeLevel: [],
	...
	$scope.addQuestion = function () {
		...
		if (!_.isUndefined($scope.config.selectedConfig.itemData.IsDecimal)) {
			newItem.IsDecimal = ($scope.config.selectedConfig.itemData.IsDecimal.toLowerCase() == ('y' || 'yes')) ? true : false;
		}
		if (!_.isUndefined($scope.config.selectedConfig.itemData.IsFraction)) {
			newItem.IsFraction = ($scope.config.selectedConfig.itemData.IsFraction.toLowerCase() == ('y' || 'yes')) ? true : false;
		}
		config.locales = {
		...
		$http.get(commoni18nFile).then(function (response) {
			...
			$http.get(localeFile).then(function (response) {
				...
				newItem.title = config.title = localeData[contentLocaleVal][$scope.config.selectedConfig.itemData.QuestionText];
	...
	var init = function () {
		...
		$scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
			instance.manifest.ver, '/editor/compare.numbers.json');
