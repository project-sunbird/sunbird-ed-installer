 ...
 */
ftEditorApp.controller('spnibController', ['$scope', '$http', 'instance', 'configs', function ($scope, $http, instance, configs) {
	...
	$scope.addQuestion = function () {
		...
		if ($scope.config.selectedConfig.itemData.Type == "Number-In-Between") {
		...
		newItem.model.mask = ($scope.config.selectedConfig.itemData.Type.indexOf("Predecessor") < 0) ? [2] : [1];
		newItem.model.qtype = ($scope.config.selectedConfig.itemData.Type.indexOf("Predecessor") >= 0) ? "Predecessor" :
			($scope.config.selectedConfig.itemData.Type.indexOf("Successor") >= 0 ? "Successor" : "Number-In-Between");
