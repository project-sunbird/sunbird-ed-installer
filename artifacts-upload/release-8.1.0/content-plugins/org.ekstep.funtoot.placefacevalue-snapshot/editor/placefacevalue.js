 ...
 */
ftEditorApp.controller('PlaceFaceValueController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
	$scope.items = [];
	var config = {
		title: 'Place value and face value of numbers',
		count: 0
	};
	var item = {
		langid: 'en',
		identifier: 'placefacevalue',
		language: ["English"],
		qid: '',
		subject: "NUM",
		grade: 0,
	...
	$scope.addQuestion = function () {
		...
		newItem.model.evaluator = $scope.config.selectedConfig.itemData.Evaluator;
		// populate step
		newItem.model.step = $scope.config.selectedConfig.itemData.Steps;
		newItem.model.placeOrFaceValue = $scope.config.selectedConfig.itemData.PlaceOrFaceValue;
	...
	var init = function () {
		...
		$scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
			instance.manifest.ver, '/editor/placefacevalue.json');
