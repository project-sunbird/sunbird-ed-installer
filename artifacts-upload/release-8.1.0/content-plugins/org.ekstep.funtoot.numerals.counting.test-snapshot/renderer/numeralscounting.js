...
org.ekstep.funtoot.common.extend({
    _type: 'org.ekstep.funtoot.numerals.counting.test',
    initPlugin: function (data) {
        ...
        if (!variables.isProcessed && !data.isSolution) {
        ...
        var numArray = helper.replaceVariables(this._item.getModelValue("numbers"), variables);
        ...
        for (r = 0; r < rows; r++) {
            ...
            for (c = 0; c < cols; c++)
                newRow.cols.push({ id: newRow.id + 'c' + c, type: "column", w: colsPerRow });
        ...
        var maskedArray = helper.getUserInputMask({ rows: rows, cols: cols, mask: mask });
        if (!data.isSolution) {
        ...
        for (r = 0; r < rows; r++) {
            for (c = 0; c < cols; c++) {
                ...
                PluginManager.invoke('mathedit', fib, cell, instance._stage, instance._theme);
            }
        }
