//@ sourceURL=subtractionhorizontal-editor.js
'use strict';
/**
 ...
 */
ftEditorApp.controller('SubtractionHorizontalController', ['$scope', '$http', 'instance', function ($scope, $http, instance) {
	$scope.items = [];
	var config = {
		title: 'Horizontal Subtraction',
		count: 0
	};
	var item = {
		langid: 'en',
		identifier: 'subtractionhorizontal',
		language: ["English"],
		qid: '',
		subject: "NUM",
		grade: 0,
		...
		type: 'ftb',
		template_id: '',
		template: 'subtractionhorizontal',
		title: '',
		question_audio: '',
		question_image: '',
	...
	$scope.addQuestion = function () {
		...
		newItem.model.missingSubtrahend = $scope.config.selectedConfig.itemData.MissingSubtrahend;
	...
	var init = function () {
		...
		$scope.config.configFile = ecEditor.resolvePluginResource(instance.manifest.id,
			instance.manifest.ver, '/editor/subtractionhorizontal.json');
