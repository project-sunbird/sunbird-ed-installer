...
kubectl create configmap keycloak-kids-keys -n $NAMESPACE \
  ...
  --from-literal=KEYCLOAK_PUBLIC_KEY_KID="$KEYCLOAK_PUBLIC_KEY_KID" -o yaml --dry-run=client | kubectl apply -f --server-side -
kubectl create configmap keycloak-key -n $NAMESPACE \
  --from-literal=$KEYCLOAK_PUBLIC_KEY_KID="$KEYCLOAK_PUBLIC_KEY" -o yaml --dry-run=client | kubectl apply -f --server-side -
